...
// Поиск неправильных привязок преподавателей в aspirantura_ruk_asp
...
// Загружаем все записи из aspirantura_ruk_asp (только активные, если deleted имеет значение)
$aspiranturaRows = GetTable('aspirantura_ruk_asp', "`deleted` = '0'");

$errors = [];
$totalChecked = 0;

foreach ($aspiranturaRows as $row) {
...
if (count($errors) > 0) {
    ...
    foreach ($errors as $i => $err) {
        echo ($i + 1) . ". aspirantura_ruk_asp.uid: {$err['asp_uid']}\n";
        echo "   Аспирант: {$err['aspirant_fio']}\n";
    ...
    // Также выводим в виде таблицы для удобного копирования uid'ов
    echo "=== UID записей для исправления ===\n";
    echo implode("\n", array_column($errors, 'asp_uid')) . "\n\n";
