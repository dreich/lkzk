...
// Поиск неправильных привязок преподавателей в aspirantura_kand_exam
...
// Загружаем все записи из aspirantura_kand_exam (только активные)
$kandExamRows = GetTable('aspirantura_kand_exam', "`deleted` = '0'");

$errors = [];
$totalChecked = 0;

foreach ($kandExamRows as $row) {
    ...
    if (!isset($bestByPersonId[$pid])) {
        $errors[] = [
            'exam_id'         => $row['id'],
            'disc_abr'        => $row['disc_abr'],
            'disc_title'      => $row['disc_title'],
            'exam_semester'   => $row['exam_semester'],
            'groups'          => $row['groups'],
            'person_id'       => $pid,
...
if (count($errors) > 0) {
    ...
    foreach ($errors as $i => $err) {
        echo ($i + 1) . ". aspirantura_kand_exam.id: {$err['exam_id']}\n";
        echo "   Дисциплина: {$err['disc_abr']} — {$err['disc_title']}\n";
        echo "   Семестр: {$err['exam_semester']}, группы: {$err['groups']}\n";
