...
{
  $res = $mysqli->query("SELECT UID FROM xml_faculty #WHERE UID IN ('25031.281474976710668', '25031.281474976710778', '25031.281474976710801', '25031.281474976710809', '25031.281474976710964')");
...
foreach ($faculty_uids_to_process as $current_faculty_uid) 
{
  ...
  foreach ($groupedData as $uid => $row)
  {
    ...
    $rowData[0] = $row['base_uid'] . ' ' . $row['FacultyOwnerAbbr']; // A
