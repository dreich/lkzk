...
## 2025-11-20 — «Учёт реальных завкафов»
- Уточнена проверка роли заведующего кафедрой: `Authorize()` теперь учитывает только подразделения, где `has_real_chief = 1`, чтобы исключить формальных руководителей @functions.php.
- Добавлен разовый скрипт `once/fix_chiefs.php`, сбрасывающий данные завкафа в нагрузке для подразделений без реального руководителя, чтобы синхронизировать данные после обновления справочника @once/fix_chiefs.php.
