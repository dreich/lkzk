...
.controller ('NagruzkaCtrl', function($rootScope, $scope, $http, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, ngDialog, $templateCache, nagruzka_type, nagruzka_selected_chair_id, $resource, $cookies, system_mode, nagruzka_stat, nagruzka, lecturer_uid)
{
  ...
  $scope.nagruzka = nagruzka ? nagruzka.data.nagruzka : null;
  $scope.nagruzka_stat = nagruzka ? nagruzka.data.stat : null;
  ...
  $scope.UpdateNagruzkaStat = function(nagr_type)
  {
    CL('UpdateNagruzkaStat');

    if (nagr_type == 'discipline')
    {
      $http({url: 'ajax/get/nagruzka_discipline.php?chair_id=' + c_chair_id, method: 'GET'})
      .then(function (response) 
      {
        if (response.data)
        {
          $scope.nagruzka_stat = response.data.stat;
  ...
  {
    $scope.UpdateNagruzkaStat('discipline');
  }
